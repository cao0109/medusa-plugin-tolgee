...
import { SupportedModels, TolgeeAdminOptions } from "../../common";
...
const TranslationManagement = ({ id, slug, availableLanguages }: Props) => {
  ...
  const { mutateAsync: syncTranslation, isPending: syncing } = useMutation({
    ...
    onSuccess: () => {
      toast.success("Success", {
        description: "Translations sync successful.",
      });
  ...
  const { mutateAsync: addTranslation, isPending: adding } = useMutation({
    ...
    onError: (error) => {
      console.error(
        `Failed to create translation for ${slug}(${id}):`,
        error.message
      );
  ...
  const { data: { keyNames = [] } = {}, isLoading } = useQuery<{
    ...
    queryFn: async () =>
      await sdk.client.fetch(`/admin/tolgee/translation/${slug}/${id}`),
  ...
  return (
    <Container>
      <Header
        ...
        subtitle={
          keyNames?.length > 0
            ? adminT("widget.subtitle")
            : adminT("widget.subtitleEmpty")
        }
        actions={
          keyNames?.length <= 0
            ? [addTranslationAction, syncAllAction]
            : [
                {
                  type: "custom",
                  children: (
                    ...
                    >
                      <Select.Trigger>
                        <Select.Value
                          placeholder={adminT("widget.selectLanguage")}
                        />
                      </Select.Trigger>
                      <Select.Content>
      ...
      {isLoading ? (
        <SectionRow title={adminT("loading")} />
      ) : (
